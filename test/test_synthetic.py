...
def update_model(wm_file: str, wm_eq_type: str, wm_dir: str = 'weather_files_synth'):
    ...
    """
    assert wm_eq_type in 'hydro wet_linear wet_nonlinear', 'Set  wm_eq_type to hydro, wet_linear, or wet_nonlinear'
...
class StudyArea:
    ...
    def __init__(self, region: str, wmName: str, path: str) -> None:
        ...
        wm_bounds = aoi.calc_buffer_ray(
            self.los.getSensorDirection(),
            lookDir=self.los.getLookDirection(),
        )
    ...
    def setup_region(self) -> None:
        ...
        if self.region == 'LA':
            ...
            self.orbit = self.orb_dir + '/S1B_OPER_AUX_POEORB_OPOD_20210317T025713_V20200129T225942_20200131T005942.EOF'

        # Fortaleza, Brazil; Ascending
        elif self.region == 'Fort':
            self.SNWE = -4.0, -3.5, -38.75, -38.25
            self.dt = datetime(2019, 11, 17, 20, 51, 58)
            self.orbit = self.orb_dir + '/S1A_OPER_AUX_POEORB_OPOD_20210315T014833_V20191116T225942_20191118T005942.EOF'

        # Utqiagvik, Alaska; Descending
        elif self.region == 'AK':
            self.SNWE = 70.25, 71.50, -157.75, -155.55
            self.dt = datetime(2022, 8, 29, 17, 0, 1)
            self.orbit = self.orb_dir + '/S1A_OPER_AUX_POEORB_OPOD_20220918T081841_V20220828T225942_20220830T005942.EOF'
...
def test_dl_real(tmp_path, region, mod='ERA5') -> None:
    ...
    with pushd(tmp_path):
        ...
        dct_cfg['runtime_group']['weather_model_directory'] = op.dirname(SAobj.path_wm_real)
...
def test_wet_eq_linear(tmp_path, region, mod='ERA-5') -> None:
    ...
    with pushd(tmp_path):
        ...
        # get the just created synthetic delays
        wm_name = SAobj.wmName.replace('-', '')  # incase of ERA-5
        ds = xr.open_dataset(f'{SAobj.wd}/{wm_name}_tropo_{SAobj.dts.replace("_", "")}_ray.nc')
        da = ds['wet']
        ds.close()
        del ds
