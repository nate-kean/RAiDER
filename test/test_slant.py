...
def test_slant_proj(weather_model_name) -> None:
    ...
    grp = {
        ...
        'los_group': {
            ...
            'orbit_file': os.path.join(
                ORB_DIR, 'S1B_OPER_AUX_POEORB_OPOD_20210317T025713_V20200129T225942_20200131T005942.EOF'
            ),
        },
    }
    ...
    path_delays = os.path.join(SCENARIO_DIR, make_delay_name(weather_model_name, date, time, 'std'))
...
def test_ray_tracing(weather_model_name) -> None:
    ...
    path_delays = os.path.join(SCENARIO_DIR, make_delay_name(weather_model_name, date, time, 'ray'))
    with xr.open_dataset(path_delays) as ds:
        delay = (ds['hydro'] + ds['wet']).sel(y=lat, x=lon, z=hgt, method='nearest').item()
    np.testing.assert_almost_equal(val, delay)
