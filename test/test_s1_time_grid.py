...
def test_s1_timing_array_wrt_slc_center_time(
    ...
    with xr.open_dataset(gunw_azimuth_test, group=group, mode='r') as ds:
        ...
        lat = ds.latitudeMeta.data - res_y / 2.0
        lon = ds.longitudeMeta.data - res_x / 2.0
    ...
    mocker.patch('RAiDER.s1_azimuth_timing._asf_query', return_value=slc_id_dict_for_azimuth_time_test[ifg_type])
...
def test_s1_timing_array_wrt_variance(
    gunw_azimuth_test: Path,
    ifg_type: str,
    orbit_dict_for_azimuth_time_test: dict,
    slc_id_dict_for_azimuth_time_test: dict,
...
def test_n_closest_dts():
    ...
    out = get_n_closest_datetimes(dt, n_target_datetimes, time_step)
    expected = [
        datetime.datetime(2023, 2, 1, 8, 0, 0),
    ...
    out = get_n_closest_datetimes(dt, n_target_datetimes, time_step)
    expected = [
        datetime.datetime(2023, 1, 1, 20, 0, 0),
...
@pytest.mark.parametrize(
    'input_time, temporal_window, expected_weights',
    zip(input_times, windows, expected_weights_list),
)
def test_inverse_weighting(
    ...
    out_weights = get_inverse_weights_for_dates(
        timing_grid,
        dates,
